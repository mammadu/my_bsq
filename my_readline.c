...
{
    int i = 0;
    while(source_string[i] != delimiter && i < my_strlen(source_string))
    {
        destination_string[i] = source_string[i];
        i++;
    }    
    source_string = &source_string[i + 1];
}

char *my_readline(int fd) 
{
    static char string_storage_block [1000000];   
    string_storage_block[0] = '\0';
    ...
    my_strcpy(string_storage_block, temp_combined_string);
    free(temp_string_storage_block);
    ...
    while (is_newline_in_string(string_storage_block) == 0 && characters_read > 0)
    {
        ...
        characters_read = read(fd, temp_string_storage_block, READLINE_READ_SIZE);
        temp_combined_string = combine_strings(string_storage_block, temp_string_storage_block);
        my_strcpy(string_storage_block, temp_combined_string);
        free(temp_string_storage_block);
    }
